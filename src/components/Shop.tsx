...
import { Gift, QrCode, X, Sparkles, Gem, Coffee, Box, Package, Check, Loader2 } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    if (selectedProduct) {
      ...
      interval = setInterval(async () => {
        try {
          // 注意：此处换成您实际的后端检测接口地址
          const res = await fetch(`/api/payment/status?amount=${selectedProduct.amount}`);
          if (res.ok) {
            const data = await res.json();
            if (data.status === 'success') {
              setIsVerifying(false);
              clearInterval(interval);
              if (selectedProduct.callback) {
          ...
          >
            ...
            >
              ...
              <div className="mt-6 flex flex-col items-center justify-center space-y-2">
                {isVerifying && <Loader2 className="w-5 h-5 text-emerald-400 animate-spin" />}
                <p className="text-xs text-white/70">支付成功后将自动跳转</p>
              </div>
            </motion.div>
          </motion.div>
        )}
